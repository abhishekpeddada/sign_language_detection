...
if not os.path.exists('dataset/train/'+a):
...
(80, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 2)
    ...
    #Time to configure the hand specifically into the ROI...
        ...
        if hand is not None:
            ...
            cv2.putText(frame_copy, str(num_frames)+"For" + a,
      ...
      + a, (200, 400), cv2.FONT_HERSHEY_SIMPLEX, 1,
      (0,0,255), 2)

            # Displaying the thresholded image
            cv2.imshow("Thresholded Hand Image", thresholded)
            if num_imgs_taken <= 60:
                #cv2.imwrite('dataset/test/'+a+'/'+a+str(num_imgs_taken)+'-frame-' + '.jpg', frame)
                cv2.imwrite('dataset/train/'+a+'/'+a+str(num_imgs_taken) + '.jpg', thresholded)
